...
const CustomTinymceField = () => {
  ...
  const onInit = (evt, editor) => {
    ...
    setCharacterLength(editor.getContent({ format: "text" }).length);
  }
  ...
  const onEditorChangeHandler = (newValue, editor) => {
    ...
    setCurrentValue(value);
    setCharacterLength(currentLength);
  };
      ...
      {fieldData.ready &&
        <View>
          <div className={'rte-top-block'}>
            ...
            {
              characterLimit &&
              <p className={'rte-character-limits'}>{characterLength}/{characterLimit}</p>
            }
          </div>
          <Editor
            init={{
