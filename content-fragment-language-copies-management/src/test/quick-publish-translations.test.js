...
const { quickPublishTranslations } = require('../actions/translations.js');
jest.mock('../actions/translations.js', () => ({
  ...jest.requireActual('../actions/translations.js'),
  quickPublishTranslations: jest.fn(),
}));

const action = require('./../actions/quick-publish-translations/index.js');
...
describe('quick-publish-translations', () => {
  ...
  test('should return an http response with the fetched content', async () => {
    quickPublishTranslations.mockResolvedValue(Promise.resolve({}));
  ...
  test('if there is an error should return a 500 and log the error', async () => {
    ...
    quickPublishTranslations.mockRejectedValue(fakeError);
